...
interface ISmallSetPaginationSearch {
  list_page: (term: string, currenPage: number) => void;
  list: [];
  count: number;
  term: string;
}

function SmallSetPaginationSearch({
  list_page,
  list,
  count,
  term,
}: ISmallSetPaginationSearch) {
  ...
  const getNumbers = () => {
    const itemsPerPage = listingsPerPage;
    let pageNumber = 1;
